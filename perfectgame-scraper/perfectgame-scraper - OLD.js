...
async function captureAllStatsTables(page, statsTablesDir, teamName) {
  ...
  const views = [
    { category: "Batting Stats", clickCategory: "Batting Stats", view: "Standard", suffix: "PG-Batting-Stats-Standard", label: "batting-standard" },
    { category: "Batting Stats", clickCategory: "Batting Stats", view: "Advanced", suffix: "PG-Batting-Stats-Advanced", label: "batting-advanced" },
    { category: "Batting Stats", clickCategory: "Batting Stats", view: "Batted Ball", suffix: "PG-Batting-Stats-Batted-Ball", label: "batting-batted-ball" },
    { category: "Pitching Stats", clickCategory: "Pitching Stats", view: "Standard", suffix: "PG-Pitching-Stats-Standard", label: "pitching-standard" },
    { category: "Pitching Stats", clickCategory: "Pitching Stats", view: "Advanced", suffix: "PG-Pitching-Stats-Advanced", label: "pitching-advanced" },
    { category: "Pitching Stats", clickCategory: "Pitching Stats", view: "Batted Ball", suffix: "PG-Pitching-Stats-Batted-Ball", label: "pitching-batted-ball" }
  ];

  for (const item of views) {
    await selectStatsView(page, item.clickCategory, item.view);

    const outputPath = path.join(statsTablesDir, `${safeTeam}-${item.suffix}.png`);
    const result = await captureCurrentStatsGrid(page, outputPath, item.label, item.category, item.view);
...
async function captureGameBoxAndPitchByPitch(context, teamPage, game, gamesDir, teamNameForFile = "") {
  ...
  try {
    ...
    const boxCrop = await getFullBoxScoreCrop(boxPage);
    const boxPath = path.join(gameDir, `${boxMeta.boxScoreBaseName}.png`);

    const boxResult = await captureFullPageThenCrop(boxPage, boxCrop, boxPath, {
      beforeScreenshotDelayMs: PG_BOX_SCORE_SCREENSHOT_DELAY_MS
    });
    boxResult.metadata = boxMeta;

    console.log(`Saved full-page-cropped box score screenshot for game ${game.gameId}: ${boxPath}`);
    ...
    if (pitchClicked.clicked) {
      ...
      const pitchChunksDir = path.join(gameDir, "pitch-by-pitch-chunks");

      pitchResult = await capturePitchByPitchChunks(
        boxPage,
        pitchClip,
        pitchChunksDir,
        boxMeta.pitchByPitchBaseName,
        {
          sliceHeight: PITCH_BY_PITCH_SLICE_HEIGHT,
          sliceDelayMs: PITCH_BY_PITCH_SLICE_DELAY_MS,
          overlap: Number(process.env.PG_PITCH_BY_PITCH_CHUNK_OVERLAP || 80)
        }
      );

      pitchResult.captured = Number(pitchResult.chunk_count || 0) > 0;
      pitchResult.folder = pitchChunksDir;

      if (!pitchResult.captured) {
        throw new Error(`Pitch By Pitch tab opened for game ${game.gameId}, but no pitch-by-pitch chunks were captured.`);
      }

      console.log(`Saved ${pitchResult.chunk_count} pitch-by-pitch chunk screenshot(s) for game ${game.gameId}: ${pitchChunksDir}`);
    }
...
async function captureTeamStatsTables(page, teamDir, teamName) {
  ...
  const statsTableScreenshots = await captureAllStatsTables(page, statsTablesDir, teamName);

  return {
    success: true,
    no_stats_recorded: statsTableScreenshots.filter(x => x.captured).length === 0,
