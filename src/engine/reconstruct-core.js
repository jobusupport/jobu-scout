...
/**
 * reconstruct-core.js — authoritative game-reconstruction engine (High School Slice 2B).
 *
 ...
 *
 * This is the AUTHORITATIVE implementation of every reconstruction formula it
 * contains — src/game-reconstructor.js is now a thin compatibility re-export
 * of this module, not an independent copy (see that file's own header). No
 * formula in this file differs from the legacy src/game-reconstructor.js this
 * was relocated from; the relocation itself is proven behavior-preserving by
 * running the pre-existing characterization suite
 * (test/legacy-engine-characterization.test.js) unchanged against this file
 * via src/game-reconstructor.js's re-export.
 *
 * Product rule:
 ...
 * dependency. Does not mutate its inputs. Fully deterministic.
 */
